...
function init() {
    ...
    template = ` 
                     ...
                     <button class="chat-btn-message">Hello! How can i help you ? <br /> <span class="professor">CU-SEVAK</span></button><button class="triangle"></button>
    ...
    chatBtn.addEventListener("click", () => {
      ...
      if (chatPopup.style.display == "none" && chatBtnMessage.style.display == "block" && triangle.style.display == "block" &&  mobileDevice) {
  ...
  function setBotResponse(val) {
    setTimeout(function () {
      if (val.length < 1) {
        ...
        var BotResponse = `<div class='bot-msg'><img class='bot-img' src ='${botLogoPath}' /><span class='msg'> $urlify{msg} </span></div>`;
        ...
        for (i = 0; i < val.length; i++) {
          ...
          if (val[i].hasOwnProperty("text")) {
            ...
            var BotResponse = `<div class='bot-msg'><img class='bot-img' src ='${botLogoPath}' /><span class='msg'>${urlify(
              val[i].text
            )}</span></div>`;
            $(BotResponse).appendTo(".chat-area").hide().fadeIn(1000);
          }
