...
import {
  getDataBySearch,
  getEnrolledCourses,
} from "../../../../redux/slices/Common/dashboardActions";
...
const EnrolledCourses = () => {
  ...
  const enrolledCourses = useSelector(
    (state) => state.dashboardState.enrolledCourses
  );
  const totalPages = useSelector(
    (state) => state?.dashboardState?.numberOfPages
  );
  const recordCount = useSelector(
    (state) => state?.dashboardState?.recordCount
  );
  ...
  const handleSearchText = (e) => {
    e.preventDefault();
    dispatch(
      getDataBySearch("enrolledCourses", searchText, pageSize, currentPage)
    );
  };
  ...
  return (
    <div className="enrolled-students pt-0">
      ...
      <div className="table-enrolled">
        <form
          className="enrolled-search py-1 d-flex justify-content-end align-items-center mb-2"
        ...
        >
          ...
          />
          <button className="btn-search m-0 d-flex">
            {" "}
            <FiSearch className="enrolled-search-icon" />
          </button>
        </form>
        ...
        {console.log(enrolledCourses, "ENROLLLLLLLL")}
        {!loadingState ? (
          content
        ) : (
          <>
