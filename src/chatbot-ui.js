...
function init() {
    ...
    template = ` <button className='chat-btn'>
    <img src = "./icons/comment.png" className = "icon" ></button>

    <div className='chat-popup'>
    
		<div className='chat-header'>
			<div className='chatbot-img'>
				<img src='${botLogoPath}' alt='Chat Bot image' className='bot-img'> 
			</div>
			<h3 className='bot-title'>Covid Bot</h3>
			<button className = "expand-chat-window" ><img src="./icons/open_fullscreen.png" className="icon" ></button>
		</div>

		<div className='chat-area'>
            <div className='bot-msg'>
                <img className='bot-img' src ='${botLogoPath}' />
            ...
            <!-- <div className='bot-msg'>
                ...
                <div className='response-btns'>
                    <button className='btn-primary' onclick= 'userResponseBtn(this)' value='/sign_in'>sample btn</button>            
                </div>
			</div> -->

			<!-- <div className='bot-msg'>
				<img className='msg-image' src = "https://i.imgur.com/nGF1K8f.jpg" />
			</div> -->
		...
		<div className='chat-input-area'>
			<input type='text' autofocus className='chat-input' onkeypress='return givenUserInput(event)' placeholder='Type a message ...' autocomplete='off'>
			<button className='chat-submit'><i className='material-icons'>send</i></button>
		</div>
    ...
    chatBtn.addEventListener("click", () => {
        ...
        if (chatPopup.style.display == "none" && mobileDevice) {
            ...
            chatInput.focus();
            chatBtn.innerHTML = `<img src = "./icons/close.png" className = "icon" >`
        } else if (mobileDevice) {
            chatPopup.style.display = "none"
            chatBtn.innerHTML = `<img src = "./icons/comment.png" className = "icon" >`
        } else {
            mobileView()
        }
    })
    ...
    expandWindow.addEventListener("click", (e) => {
        ...
        if (expandWindow.innerHTML == '<img src="./icons/open_fullscreen.png" className="icon">') {
            expandWindow.innerHTML = `<img src = "./icons/close_fullscreen.png" className = 'icon'>`
            root.style.setProperty('--chat-window-height', 80 + "%");
            root.style.setProperty('--chat-window-total-width', 85 + "%");
        } else if (expandWindow.innerHTML == '<img src="./icons/close.png" className="icon">') {
            chatPopup.style.display = "none"
            chatBtn.style.display = "block"
        } else {
            expandWindow.innerHTML = `<img src = "./icons/open_fullscreen.png" className = "icon" >`
...
function setUserResponse() {
    ...
    if (userInput) {
        let temp = `<div className="user-msg"><span className = "msg">${userInput}</span></div>`
...
function setBotResponse(val) {
    setTimeout(function() {
        if (val.length < 1) {
            ...
            var BotResponse = `<div className='bot-msg'><img className='bot-img' src ='${botLogoPath}' /><span className='msg'> ${msg} </span></div>`;
            ...
            for (i = 0; i < val.length; i++) {
                ...
                if (val[i].hasOwnProperty("text")) {
                    ...
                    var BotResponse = `<div className='bot-msg'><img className='bot-img' src ='${botLogoPath}' /><span className='msg'>${val[i].text}</span></div>`;
                    $(BotResponse).appendTo('.chat-area').hide().fadeIn(1000);
                }

                //check if there is image
                if (val[i].hasOwnProperty("image")) {
                    var BotResponse = "<div className='bot-msg'>" + "<img className='bot-img' src ='${botLogoPath}' />"
                    '<img className="msg-image" src="' + val[i].image + '">' +
                        '</div>'
                    $(BotResponse).appendTo('.chat-area').hide().fadeIn(1000);
                }
                ...
                if (val[i].hasOwnProperty("buttons")) {
                    var BotResponse = `<div className='bot-msg'><img className='bot-img' src ='${botLogoPath}' /><div className='response-btns'>`

                    buttonsArray = val[i].buttons;
                    buttonsArray.forEach(btn => {
                        BotResponse += `<button className='btn-primary' onclick= 'userResponseBtn(this)' value='${btn.payload}'>${btn.title}</button>`
                    })
...
function mobileView() {
    ...
    if (chatPopup.style.display == "none") {
        ...
        expandWindow.innerHTML = `<img src = "./icons/close.png" className = "icon" >`
    }
}
